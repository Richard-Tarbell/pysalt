...
class InterIdentifyWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, xarr, specarr, slines, sfluxes, ws, hmin=150, wmin=400, mdiff=20,
        ...
        #regionChange = QtCore.Signal(int,int)
        self.imagePage.regionChange.connect(self.regionChange)

        #runAuto = QtCore.Signal(int, int, int)
        self.imagePage.runAuto.connect(self.runauto)

        #saveWs = QtCore.Signal()
        self.arcPage.saveWs.connect(self.saveWS)

        #quit = QtCore.Signal()
        self.arcdisplay.quiT.connect(self.close)
...
class imageWidget(QtWidgets.QWidget):
    regionChange = QtCore.Signal(int,int) # Richard
    runAuto = QtCore.Signal(int, int, int) # Richard
    ...
    def updatesection(self):
        ...
        self.regionChange.emit(self.y1, self.y2)
    ...
    def runauto(self):
        ...
        ## BEGIN - RICHARD
        self.runAuto.emit(self.y1,self.nrows,self.rstep)
        """
        self.emit(
...
class arcWidget(QtWidgets.QWidget):
    ...
    def __init__(self, arcdisplay, hmin=150, wmin=450, name=None,
        ...
        print("W1", self.arcdisplay.ws.value)
        ...
        ## CHECK THE CLASS ARCDISPLAY

        ## BEGIN - RICHARD

        #keyPressEvent = QtCore.Signal()
        self.arcdisplay.keyPressEvent.connect(self.keyPressEvent)

        #updatex = QtCore.Signal(float)
        self.arcdisplay.updatex.connect(self.updatexlabel)

        #activated = QtCore.Signal(str)
        self.funcComboBox.activated.connect(self.updatefunction)

        #self.connect(self.methodComboBox, QtCore.SIGNAL('activated(QString)'), self.updatemethod) 
        self.methodComboBox.activated.connect(self.updatemethod)
    ...
    def runauto(self):
        ...
        print("Jogging")
        print("rms: ", self.arcdisplay.rms)
        print("ndstep: ", self.arcdisplay.ndstep)
...
class errWidget(QtWidgets.QWidget):
    ...
    def __init__(self, arcdisplay, hmin=150, wmin=450, name=None, parent=None):
        ...
        #fitUp = QtCore.Signal()
        self.arcdisplay.fitUp.connect(self.fitUpdate)
        """
        self.connect(
            self.arcdisplay,
...
class ArcDisplay(QtWidgets.QWidget):
    ## - BEGIN - Richard
    keyPressEvent = QtCore.Signal(str)
    updatex = QtCore.Signal(float)
    activated = QtCore.Signal(str)
    fitUp = QtCore.Signal()
    quiT = QtCore.Signal()
    positionSelected = QtCore.Signal(float,float)
    fitUpdate = QtCore.Signal()
    quiT = QtCore.Signal()
    ## END - RICHARD
 ...
 """
    ...
    def onKeyPress(self, event): # UGH
        ...
        if event.key == '?':
            ...
            if event.xdata:
                ...
                self.updatex.emit(cx) # RICHARD
        elif event.key == 'x':
            # return the x position
            if event.xdata:
                self.log.message(str(event.xdata), with_header=False)
                #self.emit(QtCore.SIGNAL("updatex(float)"), event.xdata)
                self.updatex.emit(event.xdata) # RICHARD
        elif event.key == 'R':
            # reset the fitp
            self.reset()
        elif event.key == 'f':
            ...
            self.keyPressEvent.emit(event.key) # RICHARD ADDED
            #self.emit(QtCore.SIGNAL("keyPressEvent(string)"), event.key) # RICHARD COMMENTED
    ...
    def onButtonPress(self, event):
        ...
        if event.xdata and event.ydata:
            self.positionSelected.emit(float(event.xdata),float(event.ydata)) # RICHARD - BEgIN
            #self.emit(QtCore.SIGNAL("positionSelected(float, float)"),
            #          float(event.xdata), float(event.ydata)) # Richard - end
    ...
    def findfit(self):
        print("w1 Values: ",self.xp) # RICHARD
        #print(self.ws.order)
    ...
    def err_redraw_canvas(self, keepzoom=False):
        ...
        #self.emit(QtCore.SIGNAL("fitUpdate()"))
        self.fitUpdate.emit() # RICHARD - EDIT look at ArcDisplay
